...
def extrair_metricas_criterios(df: pd.DataFrame) -> dict[str, int]:
    ...
    flag_pacote = serie_tipo_os_1.str.contains(TERMO_MIGRACAO_OS, na=False, regex=False)
    flag_gpon_sim = serie_flag_gpon.eq(VALOR_FLAG_GPON_SIM)
    flag_pon = serie_habilidade.str.contains(
...
def render_painel_criterios(df: pd.DataFrame) -> None:
    ...
    with tab_painel:
        ...
        st.caption(
            "Os segmentos abaixo representam a classificação final da base. Os critérios individuais podem se sobrepor entre si."
        )
        ...
        st.markdown("<br>", unsafe_allow_html=True)
        st.info(
            f"🔦 HABILIDADE contendo `{TERMO_GPON_HABILIDADE}`: **{fmt(metricas['criterio_habilidade_pon'])} tarefas**."
        )
    ...
    with tab_detalhes:
        ...
        tarefas = _serie_total_tarefas(df)
        distribuicao = (
            df.assign(_TAREFAS=tarefas, _TIPO_FINAL=df["TIPO_SERVICO"].fillna("Outros"))
            .groupby("_TIPO_FINAL", dropna=False)
            .agg(Tarefas=("_TAREFAS", "sum"), Registros=("_TIPO_FINAL", "size"))
            .reset_index()
        ...
        distribuicao["% das Tarefas"] = np.where(
            total_tarefas > 0, distribuicao["Tarefas"] / total_tarefas, 0
        )
        st.dataframe(
        ...
        if metricas["criterio_and_migracao"] == 0:
            if metricas["criterio_pacote"] == 0:
                ...
                st.warning(
                    "⚠️ Existem tarefas com Mudança de Pacote e tarefas GPON, mas nenhum registro atende aos dois juntos."
                )
        else:
            st.success(
                f"✅ {fmt(metricas['criterio_and_migracao'])} tarefas atendem simultaneamente aos critérios de Migração."
            )
        ...
        if metricas["criterio_and_pme"] == 0:
            ...
            st.success(
                f"✅ {fmt(metricas['criterio_and_pme'])} tarefas atendem simultaneamente aos critérios de PME."
            )
