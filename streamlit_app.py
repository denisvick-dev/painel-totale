...
class ProcessadorDeDados:
    ...
    def _normalizar_serie(serie: pd.Series) -> pd.Series:
        return serie.astype(str).str.strip().replace(list(Configuracoes.VAZIOS), "").fillna("")
    ...
    def tratar_planos_vetorizado(df: pd.DataFrame) -> pd.DataFrame:
        ...
        tv_bruta = df["PLANO TV"].astype(str).str.strip().replace("SERVIÇOS AVANÇADOS", "CLARO TV+ BOX")
        ...
        tv_do_internet = ProcessadorDeDados._normalizar_serie(partes[1]) if partes.shape[1] > 1 else pd.Series("", index=df.index, dtype=str)
        tv_normalizada = ProcessadorDeDados._normalizar_serie(tv_bruta)

        tv_limpa = pd.Series(np.where(tv_normalizada != "", tv_normalizada, tv_do_internet), index=df.index, dtype=str)

        tem_tv = tv_limpa != ""
        tem_internet = internet_limpa != ""

        df["QTDE_CONSULTIVO"] = tem_tv.astype(int) + tem_internet.astype(int)
        condicoes = [tem_tv & tem_internet, tem_tv & ~tem_internet, ~tem_tv & tem_internet]
...
def pagina_home():
    ...
    if not ("dados_prod" in st.session_state and st.session_state["dados_prod"] is not None):
        st.markdown("""
    ...
    with col1:
        st.markdown('<div class="card"><h4>⚙️ Produção</h4>Monitore eficiência operacional, volume produzido e desempenho.</div>', unsafe_allow_html=True)
    with col2:
        st.markdown('<div class="card"><h4>📈 Indicadores Estratégicos</h4>Acompanhe metas, resultados financeiros e KPIs.</div>', unsafe_allow_html=True)
    ...
    with col_c2:
        ...
        st.image(Configuracoes.IMGS_CARROSSEL[st.session_state.slide_index], use_container_width=True)
        st.markdown(f"<div style='text-align:center; font-size:14px; color:#666;'>Slide {st.session_state.slide_index + 1} de {total}</div>", unsafe_allow_html=True)
        
        btn1, btn2 = st.columns(2)
        if btn1.button("⬅ Anterior", use_container_width=True): st.session_state.slide_index = (st.session_state.slide_index - 1) % total
        if btn2.button("Próximo ➡", use_container_width=True): st.session_state.slide_index = (st.session_state.slide_index + 1) % total
    ...
    # Auto Refresh
    if "_last_refresh_time" not in st.session_state: st.session_state["_last_refresh_time"] = time.time()
...
def pagina_envio_excel():
    ...
    def executar_atualizacao():
        ...
        try:
            ...
            st.session_state["ultima_atualizacao"] = datetime.now(Configuracoes.FUSO_HORARIO)
    ...
    if "dados_cons" not in st.session_state: executar_atualizacao()

    col_btn, _ = st.columns([1, 4])
    if col_btn.button("🔄 Atualizar Agora", use_container_width=True, type="primary"):
        if executar_atualizacao(): st.success("✅ Dados atualizados com sucesso!")
    ...
    hora_str = ultima_data.strftime("%d/%m/%Y às %H:%M:%S") if ultima_data else "Nunca atualizado"

    aba1, aba2 = st.tabs(["📊 Pré-visualização: Produção", "📋 Pré-visualização: Consultivos"])

    with aba1:
        c1, c2, c3 = st.columns(3)
        c1.markdown(Visual.criar_card("Total Registros", f"{len(df_prod):,}".replace(",", "."), "azul"), unsafe_allow_html=True)
        c2.markdown(Visual.criar_card("Total Colunas", str(len(df_prod.columns)), "cinza"), unsafe_allow_html=True)
        c3.markdown(Visual.criar_card("Última Sincronização", hora_str, "verde"), unsafe_allow_html=True)
        if not df_prod.empty: st.dataframe(df_prod.head(100), use_container_width=True, height=400, hide_index=True)
        else: st.warning("⚠️ Nenhuma aba chamada 'Prod' encontrada.")

    with aba2:
        c4, c5, c6 = st.columns(3)
        c4.markdown(Visual.criar_card("Total Registros", f"{len(df_cons):,}".replace(",", "."), "azul"), unsafe_allow_html=True)
        c5.markdown(Visual.criar_card("Total Colunas", str(len(df_cons.columns)), "cinza"), unsafe_allow_html=True)
        c6.markdown(Visual.criar_card("Última Sincronização", hora_str, "verde"), unsafe_allow_html=True)
        if not df_cons.empty: st.dataframe(df_cons.head(100), use_container_width=True, height=400, hide_index=True)
        else: st.warning("⚠️ Nenhuma aba encontrada ou o arquivo CSV está vazio.")
