...
def _render_card_status_pme(m_seg: dict[str, Any], sla_meta: float) -> None:
    ...
    header_esq = (
        ...
        '<div style="width:44px;height:44px;background:linear-gradient(135deg,#7C3AED 0%,#A855F7 100%);'
        ...
        'color:#1F2937;letter-spacing:-0.3px;line-height:1.1;">PME</div>'
        "<div style=\"font-family:'Inter',sans-serif;font-size:12px;color:#6B7280;"
        'font-weight:500;margin-top:2px;">Pequenas e Médias Empresas</div>'
        "</div>"
        "</div>"
    )
...
class PDFExecutivoPME:
    ...
    def _tab(
        ...
        wrapper = Table(
            wrapper_data, colWidths=[cls.LARGURA_UTIL * cm], hAlign="CENTER"
        )
        wrapper.setStyle(
            TableStyle(
                [
    ...
    def _capa(cls, s: Any, m: dict, sla_meta: float, total_registros: int) -> list[Any]:
        ...
        tab_banner = Table(
            banner_data, colWidths=[cls.LARGURA_UTIL * cm], hAlign="CENTER"
        )
        tab_banner.setStyle(
            TableStyle(
                [
    ...
    def gerar(
        ...
        if not df_tec.empty:
            ...
            el.append(
                Paragraph(
                    escape(txt_tec), s["PME_Alerta"] if acima > 0 else s["PME_OK"]
                )
            )
...
def _sub_pendentes(df_seg: pd.DataFrame, sla_meta: float) -> None:
    ...
    if not df_top_mon.empty:
        ...
        st.plotly_chart(
            fig_mon, use_container_width=True, config={"displayModeBar": False}
        )
    else:
    ...
    with col_exp1:
        st.download_button(
            ...
            file_name=(
                f"pendentes_pme_filtrado_{datetime.now().strftime('%Y%m%d_%H%M')}.xlsx"
            ),
    ...
    with col_exp2:
        st.download_button(
            ...
            file_name=(
                f"pendentes_pme_completo_{datetime.now().strftime('%Y%m%d_%H%M')}.xlsx"
            ),
            mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
