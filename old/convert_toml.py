...
st.set_page_config(
    page_title="Conversor de Credenciais | Totale", page_icon="🔐", layout="centered"
)
...
st.markdown(
    '<h1 class="main-title">🔐 Conversor de Credenciais Google</h1>',
    unsafe_allow_html=True,
)
st.markdown('<p class="subtitle">JSON → secrets.toml</p>', unsafe_allow_html=True)

st.info(
    "Faça upload do arquivo JSON da Service Account do Google Cloud para gerar o `secrets.toml` no formato correto."
)
        ...
        if st.button("🔌 Testar Conexão com Google Sheets"):
            with st.spinner("Autenticando..."):
                try:
                    ...
                    st.success(
                        "✅ **Autenticação bem-sucedida!** A chave JWT é válida."
                    )
                    st.balloons()

                except ImportError:
                    st.error(
                        "❌ Biblioteca não instalada. Execute: `pip install google-auth`"
                    )
                except Exception as e:
