...
if not df_bruto.empty:
    ...
    with aba_dash:
        ...
        with c_f1:
            bases_disponiveis = sorted(
                df_view["Base"].dropna().astype(str).unique().tolist()
            )
            filtro_base = st.selectbox(
                "Filtrar por Base", ["Todas"] + bases_disponiveis
            )
            if filtro_base != "Todas":
                df_view = df_view[df_view["Base"] == filtro_base]

        with c_f2:
            sits_disponiveis = sorted(
                df_view["Situação"].dropna().astype(str).unique().tolist()
            )
            filtro_sit = st.selectbox(
                "Filtrar por Situação", ["Todas"] + sits_disponiveis
            )
            if filtro_sit != "Todas":
        ...
        with c_f3:
            ...
            filtro_cont = st.selectbox(
                "Tipo de Contrato", ["Todos"] + conts_disponiveis
            )
        ...
        custo_mensal = (
            df_view["Custo_Mensal"].sum() if "Custo_Mensal" in df_view.columns else 0
        )
        ...
        if "Proxima_Manutencao" in df_view.columns:
            manutencao_pendente = int(
                (
                    df_view["Proxima_Manutencao"].notna()
                    & (df_view["Proxima_Manutencao"] < hoje)
                ).sum()
            )
        else:
        ...
        with kpi1:
            st.markdown(
                criar_card_metrica("Total Ativos", total_ativos, "cinza"),
                unsafe_allow_html=True,
            )
        with kpi2:
            st.markdown(
                criar_card_metrica("Em Operação", ativos_operando, "verde"),
                unsafe_allow_html=True,
            )
        with kpi3:
            st.markdown(
                criar_card_metrica(
                    "Custo Mensal", formatar_moeda(custo_mensal), "vermelho"
                ),
                unsafe_allow_html=True,
            )
        with kpi4:
            st.markdown(
                criar_card_metrica(
                    "Manutenção Atrasada", manutencao_pendente, "amarelo"
                ),
                unsafe_allow_html=True,
            )
        with kpi5:
            st.markdown(
                criar_card_metrica("Bases Ativas", df_view["Base"].nunique(), "azul"),
                unsafe_allow_html=True,
            )
        ...
        with st.form("form_novo_ativo", clear_on_submit=True):
            ...
            with c3:
                ...
                novo_valor_hora = st.number_input(
                    "Valor Hora (R$)", min_value=0.0, step=0.50
                )
                novo_custo = st.number_input(
                    "Custo Mensal Estimado (R$)", min_value=0.0, step=50.0
                )
    ...
    with aba_edit:
        ...
        if tecnico_sel != "":
            ...
            with col_ed, st.form("form_editar"):
                ...
                with ec1:
                    ...
                    st.text_input(
                        "Login", value=str(dados.get("Login", "")), disabled=True
                    )
                    edit_nome = st.text_input(
                        "Técnico", value=str(dados.get("Técnico", ""))
                    )
                    edit_base = st.text_input("Base", value=str(dados.get("Base", "")))

                with ec2:
                    edit_monitor = st.text_input(
                        "Monitor", value=str(dados.get("Monitor", ""))
                    )
                    ...
                    edit_situacao = st.selectbox(
                        "Situação", LISTA_SITUACOES, index=sit_idx
                    )

                    cont_atual = str(dados.get("Tipo_Contrato", "CLT"))
                    cont_idx = (
                        LISTA_CONTRATOS.index(cont_atual)
                        if cont_atual in LISTA_CONTRATOS
                        else 0
                    )
                    edit_contrato = st.selectbox(
                        "Contrato", LISTA_CONTRATOS, index=cont_idx
                    )
                    ...
                    edit_custo = st.number_input(
                        "Custo Mensal", value=custo_atual, min_value=0.0
                    )
                ...
                with em1:
                    ...
                    val_ult = (
                        val_ult.date()
                        if pd.notna(val_ult) and hasattr(val_ult, "date")
                        else None
                    )
                ...
                with em2:
                    ...
                    val_prox = (
                        val_prox.date()
                        if pd.notna(val_prox) and hasattr(val_prox, "date")
                        else None
                    )
            ...
            with col_ex:
                ...
                st.warning(
                    "Inativar remove o técnico das listas operacionais, mas mantém o histórico."
                )
