...
def carregar_producao_raw() -> tuple[pd.DataFrame, str | None]:
...
def carregar_consultivo_raw() -> tuple[pd.DataFrame, str | None]:
...
def mostrar_colunas_e_debug() -> None:
    ...
    st.info(
        "Esta função mostra todas as colunas disponíveis para ajudar no mapeamento dos dados"
    )
    ...
    if not df_prod.empty:
        ...
        with col3:
            st.metric(
                "Memória", f"{df_prod.memory_usage(deep=True).sum() / 1024:.1f} KB"
            )
        ...
        for col in df_prod.columns:
            colunas_info.append(
                {
                    "Nome Original": col,
                    "Nome Normalizado": re.sub(
                        r"[^\w\s]", "", str(col).lower().strip()
                    ).replace(" ", "_"),
        ...
        for categoria, candidatos in mapeamento_sugestoes.items():
            encontradas = []
            for col in df_prod.columns:
                col_normal = re.sub(r"[^\w\s]", "", str(col).lower().strip()).replace(
                    " ", "_"
                )
    ...
    if not df_cons.empty:
        ...
        with col3:
            st.metric(
                "Memória", f"{df_cons.memory_usage(deep=True).sum() / 1024:.1f} KB"
            )
...
def print_colunas_console() -> None:
    ...
    if erro_prod:
        ...
        for i, col in enumerate(df_prod.columns, 1):
            print(
                f"{i:3}. {col:40} | Tipo: {df_prod[col].dtype!s:15} | Únicos: {df_prod[col].nunique():6}"
            )
        ...
        for categoria, lista in candidatos.items():
            encontradas = [
                c for c in df_prod.columns if any(l in c.lower() for l in lista)
            ]
    ...
    if erro_cons:
        ...
        for i, col in enumerate(df_cons.columns, 1):
            print(
                f"{i:3}. {col:40} | Tipo: {df_cons[col].dtype!s:15} | Únicos: {df_cons[col].nunique():6}"
            )
        ...
        for categoria, lista in candidatos.items():
            encontradas = [
                c for c in df_cons.columns if any(l in c.lower() for l in lista)
            ]
            print(f"  {categoria}: {encontradas if encontradas else 'Nenhuma'}")
