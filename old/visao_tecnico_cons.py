    ...
    def padronizar_colunas(df: pd.DataFrame) -> pd.DataFrame:
        ...
        for col_origem, col_destino in mapa.items():
            if col_origem in df.columns:
                df[col_destino] = (
                    pd.to_numeric(df[col_origem], errors="coerce").fillna(0).astype(int)
                )
        return df
...
class Graficos:
    @staticmethod
    def grafico_linhas_vendas(
        df: pd.DataFrame, x_col: str, y_cons: str, y_prod: str
    ) -> go.Figure:
        fig = go.Figure()
        fig.add_trace(
            go.Scatter(
...
if df_cons is None or df_cons.empty:
    render_insight(
        "Base não encontrada na sessão. Carregando dados de demonstração.", "alerta"
    )
...
col_tec = Utilitarios.buscar_coluna(
    df_cons, ["VENDEDOR", "TÉCNICO", "TECNICO", "NOME EQUIPE", "LOGIN"]
)
col_sup = Utilitarios.buscar_coluna(
    df_cons, ["SUPERVISOR", "MONITOR", "GESTOR", "COORDENADOR", "LÍDER"]
)
col_base = Utilitarios.buscar_coluna(
    df_cons, ["BASE", "PROJETO", "CIDADE", "FILIAL", "LOCALIDADE"]
)
...
with st.container(border=True):
    ...
    with f_base:
        if col_base:
            bases = ["Todas"] + sorted(
                [
                    str(b)
                    for b in df_cons[col_base].dropna().unique()
                    if str(b).strip() != ""
                ]
            )
    ...
    with f_sup:
        if col_sup:
            monitores = ["Todos"] + sorted(
                [
                    str(m)
                    for m in df_cons.loc[mask, col_sup].dropna().unique()
                    if str(m).strip() != ""
                ]
            )
    ...
    tecnicos_filtrados = sorted(
        [t for t in df_cons.loc[mask, col_tec].unique() if t and t != "NAN"]
    )

    with col_busca:
        tec_selecionado = st.selectbox(
            "🔎 Selecione o Técnico:", options=[""] + tecnicos_filtrados
        )
    ...
    if tec_selecionado:
        ...
        sup_tec = (
            ...
            if col_sup and not df_tec[col_sup].empty
            else "Não Atribuído"
        )
        base_tec = (
            df_tec[col_base].mode()[0]
            if col_base and not df_tec[col_base].empty
            else "Não Atribuída"
        )
        ...
        with col_info:
            ...
            render_insight(
                f"<b>Gestor Comercial:</b> {sup_tec} &nbsp;|&nbsp; <b>Base:</b> {base_tec}",
                "info",
            )
...
if tec_selecionado and not df_tec.empty:
    ...
    render_kpi(
        vc2, "Vendas Fechadas", str(t_prod), "🚀 Produtos convertidos", "laranja"
    )

    cor_win = "verde" if taxa_conversao >= 0.1 else "laranja"
    render_kpi(
        vc3, "Win Rate", f"{taxa_conversao:.1%}", "📈 Taxa de Conversão", cor_win
    )
    ...
    with g_linha:
        ...
        if col_data:
            ...
            if not df_tec_grafico.empty:
                df_tempo = (
                    df_tec_grafico.groupby(col_data)[["CONSULTIVOS", "VENDAS"]]
                    .sum()
                    .reset_index()
                )
                st.plotly_chart(
                    Graficos.grafico_linhas_vendas(
                        df_tempo, col_data, "CONSULTIVOS", "VENDAS"
                    ),
                    use_container_width=True,
                )
            else:
                render_insight(
                    "Nenhuma data válida encontrada para este técnico.", "alerta"
                )
        else:
    ...
    with g_pizza:
        ...
        if not df_mix.empty:
            st.plotly_chart(
                Graficos.grafico_rosca_mix(df_mix), use_container_width=True
            )
        else:
    ...
    colunas_exibir = [
        c
        for c in [col_data, col_tec, "CONSULTIVOS", "VENDAS", "MESH", "TV", "VIRTUA"]
        if c is not None and c in df_tec.columns
    ]
    if col_data and col_data in df_tec.columns:
        df_tec[col_data] = pd.to_datetime(
            df_tec[col_data], errors="coerce", dayfirst=True
        )
    ...
    configs_tabela = {
        "CONSULTIVOS": st.column_config.NumberColumn(
            "🗣️ Consultivos", help="Total de abordagens"
        ),
        "VENDAS": st.column_config.ProgressColumn(
            "💰 Vendas", format="%d", min_value=0, max_value=max_vendas
        ),
    ...
    if col_data is not None:
        configs_tabela[col_data] = st.column_config.DateColumn(
